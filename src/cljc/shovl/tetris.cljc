...
(defn clear-filled-rows [grid]
  (let [num-cols (count (first grid))
        num-rows (count grid)
        cleared-grid (remove #(every? (partial = true) %) grid)
...
(defn score-game [board]
  (if (:deadly-command board)
    0
    (let [move-scores (:move-scores board)]
      (sum-move-scores move-scores 0))))
  ...
  (let [old-members (:members unit)
    ...
    (if-let [next-unit (first source)]
      (cond-> (-> new-board
                  ...
                  (assoc :previous-positions [next-unit])
                  (update :source rest))
        (overlaps-filled? (:grid new-board) next-unit) (assoc :game-over true))
      (assoc new-board :unit nil :previous-positions nil :game-over true))))
