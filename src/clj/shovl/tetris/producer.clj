...
(defn unit-centering [num-cols {:keys [members] :as unit}]
...
(defn load-boards [{num-cols :width :as problem}]
  (let [{num-rows :height pre-filled :filled
         ...
         problem-units :units}
        (-> problem
            (update :units (partial map (comp #(unit-centering num-cols %)
                                              unit-map->vec)))
...
(def problem-0-boards (-> "problem_0.json" load-json-file load-boards))
